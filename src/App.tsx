...
Additional Details: ${companyForm.notes || "None"}`;
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] text-slate-900 font-sans antialiased">
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <button 
              ...
              className={`text-xs font-bold uppercase tracking-wider transition px-3 py-1.5 rounded-lg border flex items-center gap-1.5 ${
                showConfigHelper 
                  ? "bg-amber-100 text-amber-800 border-amber-200" 
                  : "text-amber-700 bg-amber-50/50 hover:bg-amber-50 border-amber-100/50 hover:text-amber-800"
              }`}
            >
              <Shield className="h-3.5 w-3.5" /> Firebase Guide
            </button>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            ...
            <button 
              onClick={() => { setShowConfigHelper(!showConfigHelper); setMobileMenuOpen(false); }} 
              className="block w-full text-left py-2 font-bold uppercase tracking-wider text-xs text-amber-700 hover:text-amber-950 flex items-center gap-1.5"
            >
              <Shield className="h-4 w-4" /> Firebase Guide
            </button>
      ...
      {/* Firebase Custom Configuration Assistant */}
      <AnimatePresence>
        {(showConfigHelper || firebaseError) && (
          <motion.div 
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="bg-amber-50 border-b border-amber-200 px-4 py-6 sm:px-6 lg:px-8 relative z-30"
          >
                ...
                <div className="flex-1 space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                      Firebase Custom Configuration Assistant
                      <span className="text-[10px] bg-amber-200 text-amber-900 px-2 py-0.5 rounded-full font-extrabold uppercase">Setup Guide</span>
                    </h3>
                    <button 
                      onClick={() => { setShowConfigHelper(false); setFirebaseError(null); }}
                  ...
                  {firebaseError && (
                    <div className="bg-white border border-amber-200 rounded-lg p-3 text-xs text-amber-900 space-y-1">
                      ...
                      <p className="font-mono bg-slate-50 p-2 rounded border border-slate-100 text-[11px] overflow-x-auto whitespace-pre-wrap">
                        {firebaseError.message} {firebaseError.path ? `(Collection: ${firebaseError.path})` : ''}
                      </p>
                    </div>
                  )}

                  <p className="text-slate-700 text-xs leading-relaxed">
                    You have successfully integrated your custom external Firebase project <strong className="font-semibold">"zenire-custom"</strong>! To resolve permissions errors and enable authentication from this application, please follow the two simple steps below in your Firebase Console.
                  </p>
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm space-y-3 flex flex-col justify-between">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <span className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold">1</span>
                          <h4 className="text-xs font-bold text-slate-900">Configure Firestore Security Rules</h4>
                        </div>
                        <p className="text-slate-600 text-[11px] leading-relaxed">
                          Your custom Firestore database is currently rejecting read/write access. Paste these rules into your Firebase Console under <strong className="font-semibold">Firestore Database &gt; Rules</strong>:
                        </p>
                        
                        <div className="relative">
                          <pre className="bg-slate-950 text-slate-300 text-[10px] font-mono p-3 rounded-lg overflow-x-auto max-h-[160px] text-left whitespace-pre border border-slate-800">
{`rules_version = '2';
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /jobs/{jobId} {
      allow read: if true;
      allow write: if request.auth != null && request.auth.token.email == 'swipetoconnect@gmail.com';
    }
    match /applications/{applicationId} {
      allow create: if true;
      allow read, update, delete: if request.auth != null && request.auth.token.email == 'swipetoconnect@gmail.com';
    }
    ...
    match /stats/{statId} {
      ...
      allow list, delete: if request.auth != null && request.auth.token.email == 'swipetoconnect@gmail.com';
    }
  }
}`}
                          </pre>
  ...
  match /databases/{database}/documents {
    ...
    match /stats/{statId} {
      ...
      allow list, delete: if request.auth != null && request.auth.token.email == 'swipetoconnect@gmail.com';
                        ...
                        className="w-full bg-slate-950 hover:bg-slate-800 text-white py-2 rounded-lg text-[11px] font-bold uppercase tracking-wider transition"
                      >
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm space-y-3 flex flex-col justify-between">
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <span className="inline-flex items-center justify-center h-6 w-6 rounded-full bg-blue-100 text-blue-800 text-xs font-bold">2</span>
                          <h4 className="text-xs font-bold text-slate-900">Authorize Redirect Domains</h4>
                        </div>
                        <p className="text-slate-600 text-[11px] leading-relaxed">
                          Firebase Auth blocks Google login unless the hosting domains are authorized. Go to <strong className="font-semibold">Authentication &gt; Settings &gt; Authorized Domains</strong> and add these exact domains:
                        </p>
                        
                        <div className="space-y-1.5 font-mono text-[10px] bg-slate-50 p-2.5 rounded-lg border border-slate-200 text-left">
                          <div className="flex items-center justify-between pb-1.5 border-b border-slate-100">
                            <span className="text-slate-700 select-all">ais-dev-tmv4yvdzhbcqq5o2dl2pj7-820646776895.asia-east1.run.app</span>
                            <button onClick={() => copyToClipboard("ais-dev-tmv4yvdzhbcqq5o2dl2pj7-820646776895.asia-east1.run.app", "Development Domain")} className="text-emerald-600 hover:text-emerald-700 font-sans font-semibold text-[10px] shrink-0">Copy</button>
                          </div>
                          <div className="flex items-center justify-between py-1.5 border-b border-slate-100">
                            <span className="text-slate-700 select-all">ais-pre-tmv4yvdzhbcqq5o2dl2pj7-820646776895.asia-east1.run.app</span>
                            <button onClick={() => copyToClipboard("ais-pre-tmv4yvdzhbcqq5o2dl2pj7-820646776895.asia-east1.run.app", "Shared Domain")} className="text-emerald-600 hover:text-emerald-700 font-sans font-semibold text-[10px] shrink-0">Copy</button>
                          </div>
                          <div className="flex items-center justify-between pt-1.5">
                            <span className="text-slate-700 select-all">localhost</span>
                            <button onClick={() => copyToClipboard("localhost", "Localhost")} className="text-emerald-600 hover:text-emerald-700 font-sans font-semibold text-[10px] shrink-0">Copy</button>
                          </div>
                        </div>
                      </div>
                      ...
                      <a 
                        ...
                        className="w-full text-center bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg text-[11px] font-bold uppercase tracking-wider block transition"
                      >
